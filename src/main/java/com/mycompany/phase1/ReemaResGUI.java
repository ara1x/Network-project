...
public class ReemaResGUI extends JFrame {
    ...
    private static final Color ACCENT_TURQUOISE = Color.decode("#5BA3A3");
    ...
    private void loadImages() {
        try {
            ...
            bgImage = createPlaceholderImage(1200, 750, SAND);
            poolImage1 = createPlaceholderImage(400, 300, ACCENT_TURQUOISE);
            poolImage2 = createPlaceholderImage(400, 300, DARK_SAND);
            roomImage = createPlaceholderImage(400, 300, WARM_WHITE);
            logoImage = createLogoImage();
        }
    }
    ...
    private BufferedImage createLogoImage() {
        ...
        g.fillOval(15, 10, 40, 40);

        // Draw palm
        g.setColor(DARK_SAND);
        g.fillRect(70, 30, 6, 30);
        g.setColor(new Color(122, 159, 53));
        for (int i = -2; i <= 2; i++) {
            int angle = i * 25;
            g.drawLine(73, 30,
                    73 + (int) (15 * Math.sin(Math.toRadians(angle))),
                    30 - (int) (15 * Math.cos(Math.toRadians(angle))));
        }

        // Draw text
        g.setColor(TEXT_DARK);
        g.setFont(new Font("Serif", Font.BOLD, 28));
        g.drawString("SAHARA", 100, 35);
        g.setFont(new Font("Serif", Font.PLAIN, 14));
        g.drawString("RESORT", 100, 52);
        g.setFont(new Font("Sans-Serif", Font.PLAIN, 9));
        g.setColor(DARK_SAND);
        g.drawString("ALULA • SAUDI ARABIA", 100, 65);
    ...
    private void buildHome() {
        JPanel p = new BackgroundImagePanel(bgImage);
    ...
    private void buildResults() {
        ...
        listRooms.setSelectionBackground(ACCENT_TURQUOISE);
        ...
        listRooms.setCellRenderer(new DefaultListCellRenderer() {
            ...
            public Component getListCellRendererComponent(
                ...
                if (isSelected) {
                    lbl.setBackground(ACCENT_TURQUOISE);
    ...
    // MY_BOOKINGS  (with selectable list + Cancel button)
    private void buildMyBookings() {
        ...
        bookingsList.setSelectionBackground(ACCENT_TURQUOISE);
        ...
        // NEW cancel behaviour: use selected item, no dialog
        btnCancel.addActionListener(e -> {
            ...
            try {
                ...
                if (resp.startsWith("OK CANCELED")) {
                    ...
                    refreshMyBookingsView(); // reload from server
                } else {
                    JOptionPane.showMessageDialog(this, resp);
                }
            } catch (Exception ex) {
        ...
        try {
            ...
            if (resp.startsWith("OK RES")) {
                ...
                for (int i = 0; i < items.length; i++) {
                    ...
                    // format: resId|roomId@dayxNights
                    String resId = "";
                    String info = item;
